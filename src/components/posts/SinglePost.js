              ...
              <ImageList sx={{ width: '100%', maxHeight: 400 }} cols={3} rowHeight={200}>
                {post.Images.map((image) => (
                  <ImageListItem key={image.id}>
                    <img
                      src={image.url || `https://${process.env.AWS_BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/${image.s3Key}`}
                      ...
                      onClick={() => window.open(image.url || `https://${process.env.AWS_BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/${image.s3Key}`, '_blank')}
                    />
                    <ImageListItemBar
